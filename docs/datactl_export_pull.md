...
```
datactl export pull [(--source-type SOURCE_TYPE) (--source-name SOURCE_NAME)]
```
...
```
  ...
  datactl export pull all --source-type dataService
  
  # Pull all data from a particular source.
  datactl export pull all --source-name my-dataservice-cluster
```
