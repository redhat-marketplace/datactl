...
Finalizes the download of files.

### Synopsis

Commits the file on the Dataservice.

 Committing indicates that the user will be delivering the files for processing by using the "datactl export push" command. Commiting files are recorded in the datactl config file.

```
datactl export commit [(--dry-run)]
```
