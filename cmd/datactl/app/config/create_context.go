...
func NewCmdConfigSetContext(pathOptions *clientcmd.PathOptions, streams genericclioptions.IOStreams) *cobra.Command {
	options := &createContextOptions{configAccess: pathOptions}
	...
	cmd := &cobra.Command{
		...
		Run: func(cmd *cobra.Command, args []string) {
			...
			if exists {
				fmt.Fprintf(streams.Out, "Context %q modified.\n", name)
			} else {
				fmt.Fprintf(streams.Out, "Context %q created.\n", name)
			}
		},
	}
