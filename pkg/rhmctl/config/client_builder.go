...
import (
	...
	"github.com/redhat-marketplace/rhmctl/pkg/clients/serviceaccount"
...
func (config *DeferredLoadingClientConfig) MeteringExport() (*rhmctlapi.MeteringExport, error) {
	mergedClientConfig, err := config.createClientConfig()
	if err != nil {
		return nil, err
	}
...
func NewNonInteractiveClientConfig(config rhmctlapi.Config, contextName string, overrides *ConfigOverrides, configAccess ConfigAccess, kubectl *genericclioptions.ConfigFlags) ClientConfig {
...
func (config *DirectClientConfig) DataServiceClientConfig() (*dataservice.DataServiceConfig, error) {
	...
	rhmctlConfig, err := config.RawConfig()
	if err != nil {
		return nil, err
	}

	dsConfig, exists := rhmctlConfig.DataServiceEndpoints[context.Cluster]

	if !exists {
		return nil, fmt.Errorf("data-service is not configured, run %q", "rhmctl config init")
	}
...
func (config *DirectClientConfig) MeteringExport() (*rhmctlapi.MeteringExport, error) {
	...
	currentMeteringExport, ok = rhmctlConfig.MeteringExports[context.Cluster]
	...
	if !ok {
		...
		rhmctlConfig.MeteringExports[context.Cluster] = currentMeteringExport
